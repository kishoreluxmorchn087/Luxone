      ...
      <div className="space-y-4">
        ...
        {isDraftService ? (
          <div role="status" className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            <span className="font-semibold">Draft service:</span> appointments can still be created and linked while this service is being prepared.
          </div>
        ) : null}
            ...
            <CRMSectionCard title="Appointments" action={<button type="button" onClick={() => navigate(`/services/appointments/create?service=${service.id}`)} className="text-xs font-medium text-blue-600">New Appointment</button>}>
              <div className="space-y-2">
                {appointments.length ? appointments.map((item) => (
                  ...
                  {isDraftService ? " Draft services can still have appointments mapped to them." : null}
                </div>}
              </div>
            </CRMSectionCard>
          </div>
